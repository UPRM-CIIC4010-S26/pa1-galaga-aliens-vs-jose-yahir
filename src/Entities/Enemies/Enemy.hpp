...
class Enemy {
    ...
    public:
        ...
        static void ManageEnemies(HitBox target, int& score) { // se añade score
            for (std::pair<std::pair<float, float>, Enemy*>& p : Enemy::enemies) {
                ...
                if (p.second) {
                    ...
                    for (Projectile& p2 : Projectile::projectiles) {
                        if (p2.ID != 1 && HitBox::Collision(p.second->hitBox, p2.getHitBox())) {
                            ...
                            if (p.second->health > 0) {
                                PlaySound(SoundManager::hit); // reproduce sonido ya cargado de SoundManager.hpp
                            } else{
                    ...
                    if (p.second->health <= 0) { // donde se muere el enemigo
                        score += p.second->enemyScoreValue; // Se aumenta el score
